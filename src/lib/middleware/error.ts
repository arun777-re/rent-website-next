...
export const handleValidation =  (fields:Record<string,any>) => {
    const missing = Object.entries(fields).filter(([key,value]) => value === undefined || value === null || value === '');
...
export const createResponse = (message:string,success:boolean,status:number,data?:any,
    totalPages?:number
)=>{
    return NextResponse.json({
        success,
...
export const paginationFunc =async (searchParams:URLSearchParams,model:mongoose.Model<any>,
    filter:Object = {}
)=>{
