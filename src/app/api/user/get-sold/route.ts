...
// api to get all sold properties
...
try {
    ...
    const properties = await Property.find({status:'sold'}).sort({createdAt:-1}).skip(skip).limit(limit);
    if(properties.length === 0){
        return createResponse('No available Properties to show',true,200,[])
    }
    return createResponse('Fetched sold Properties are',true,200,properties,totalPages)
} catch (error:any) {
