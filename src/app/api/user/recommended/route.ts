...
try {
    ...
    const interaction = await UserInteraction.find({userId}).sort({createdAt:-1}).limit(10);

    const preferredCategories = interaction.map(i => i.category).filter(Boolean);
    const preferredLocation = interaction.map(i => i.location).filter(Boolean);
    ...
    if(recommended.length === 0){
     const featuredProperties = await Property.find({featured:true}).sort({createdAt:-1}).limit(10);
     if(featuredProperties.length === 0){
        return createResponse('No Featured Property to show',true,200,[]);
     }

     return createResponse('Recommended Featured Properties are:',true,200,featuredProperties);

    }
        return createResponse('Recommended Featured Properties are:',true,200,recommended)
